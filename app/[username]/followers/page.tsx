...
function FollowerItem({ user, currentUserId }: { user: Profile; currentUserId: string | null }) {
    ...
    <main className="bg-black text-white min-h-screen flex flex-col items-center p-4">
      <div className="w-full max-w-xl">
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-xl font-light">Followers of @{profile.username}</h1>
        </div>
        <div className="flex flex-col gap-4">
          {followers.map(user => (
            <FollowerItem key={user.id} user={user} currentUserId={currentUserId} />
          ))}
          {followers.length === 0 && (
            <p className="text-neutral-500 text-center">No followers yet.</p>
          )}
        </div>
      </div>
    </main>
